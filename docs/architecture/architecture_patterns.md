...
* Основний сервіс робить бізнес-операцію → у тій же транзакції додає запис у таблицю `outbox`.
