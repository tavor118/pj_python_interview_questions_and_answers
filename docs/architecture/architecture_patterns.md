...
### Inversion of Control (IoC): бібліотека проти фреймворку [❄️4/100]
...
> (нижче) - один зі способів реалізувати IoC.
...
випадок IoC: створення залежностей виноситься назовні (див. нижче).



### Dependency Injection - DI - Впровадження залежності [💡22/100]
...
- **Через параметр конструктора** - найпоширеніший варіант (приклад вище): залежність
...
### Що таке ідемпотентність? [💡21/100]
...
### Bulkhead (ізоляція ресурсів) [❄️2/100]
...
- Реалізується пулом потоків/процесів, окремим connection pool на кожен сервіс або
  семафором (як вище) в асинхронному коді. У `resilience4j`/`Polly` - вбудований
...
### Multi-tenancy: моделі ізоляції даних [❄️2/100]
...
### Service Layer (Headless архітектура) [❄️4/100]
...
### Скінченний автомат (Finite State Machine) [❄️4/100]
