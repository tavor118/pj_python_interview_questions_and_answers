...
- Висока пропускна здатність на I/O-навантаженні: асинхронна модель (ASGI, `async`/`await`) під час очікування I/O звільняє event loop замість блокування воркера, тож один процес дешево тримає тисячі одночасних запитів. Прискорює саме I/O-bound роботу, не CPU-bound. Низькі накладні витрати забезпечують також C/Rust-компоненти під капотом: Starlette на Uvicorn (uvloop + httptools) і валідація на Pydantic v2 (`pydantic-core` на Rust).
...
### Що приймає `Depends()` у FastAPI? Якого типу об'єкт це з погляду Python? [💡11/100]
