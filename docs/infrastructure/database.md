...
### Що таке бази даних і для чого вони потрібні? [🔥41/100]
...
### Які типи баз даних існують? [💡21/100]
...
### Чим Реляційні БД відрізняються від NoSQL БД? [💡29/100]
...
### Наведіть приклади РБД та NoSQL БД [🔥40/100]
...
### Яка різниця між BASE і ACID? [💡13/100]
...
Найпоказовіша відмінність у роботі з рядками - `CHAR(n)` vs `VARCHAR(n)`: однакові на вигляд
типи поводяться по-різному в MySQL і PostgreSQL, тому це й класичне питання саме про деталі
реалізації. `CHAR(n)` - рядок **фіксованої** довжини (коротші значення доповнюються пробілами
до `n`), `VARCHAR(n)` - **змінної** довжини з верхньою межею `n`.

Історичне обґрунтування `CHAR` походить від двигунів із рядками фіксованої ширини (MyISAM):
коли кожен запис має сталий розмір, СУБД адресує рядок за зміщенням (`номер × розмір`), а
оновлення не спричиняє переміщення рядка. Звідси й порада "CHAR для значень однакової довжини".

У PostgreSQL цього виграшу немає: `char(n)`, `varchar(n)` і `text` зберігаються однаково
(varlena з префіксом довжини), тож `char(n)` не дає ні швидкості, ні економії місця - навпаки,
зазвичай найповільніший через blank-padding. До того ж порівняння `char` ігнорує кінцеві пробіли
(семантика `PAD SPACE`): `'abc' = 'abc   '` істинне, але значення повертається доповненим - звідси
сюрпризи. Документація PG рекомендує `text` або `varchar`, а `char(n)` уникати; обмеження довжини
здебільшого краще робити `CHECK`-констрейнтом, а не типом.

У MySQL/InnoDB різниця відчутніша: `CHAR(n)` справді зберігається доповненим до `n` (пробіли
зрізаються при читанні), `VARCHAR(n)` - із префіксом довжини (1-2 байти). Але для багатобайтових
кодувань (`utf8mb4`) `CHAR(n)` резервує до `n × 4` байтів, тож і тут перевага фіксованої ширини
здебільшого нівелюється.
...
### Які типи зв'язків бувають в БД? [💡21/100]
...
### Що таке індекси? Навіщо вони потрібні? Як вони працюють? [🔥30/100]
...
### Які популярні типи індексів існують? [🔥59/100]
...
### Складені (multi-column) індекси [💡14/100]
...
**Супровід індексів.** Індекс не безкоштовний: він сповільнює запис і займає місце, тож
надлишкові/невикористовувані індекси видаляють. Невикористані виявляють через
`pg_stat_user_indexes` (`idx_scan = 0` після достатнього часу спостереження). З часом
індекси **розпухають** (bloat) від оновлень/видалень - перебудова повертає компактність:
`REINDEX INDEX name` (або `REINDEX INDEX CONCURRENTLY` у Postgres 12+, щоб не блокувати
запис). Перед створенням індексу варто перевірити **селективність** колонки: на
низькоселективних полях (мало унікальних значень - `boolean`, стать) індекс марний, бо
вибірка повертає велику частку таблиці й планувальник усе одно обирає seq-scan.



### Що таке seq-scan? Коли він виконується, навіть якщо є індекси? [💡11/100]
...
### Що таке транзакція? Що таке ACID? [🔥53/100]
...
### Що таке рівні ізоляції транзакцій. Які вони бувають [🔥47/100]
...
### Що таке курсор і навіщо він потрібен [💡18/100]
...
### Що таке VACUUM в PostgreSQL [💡10/100]
...
### Як виявити повільний SQL запит? [💡10/100]
...
### Що таке профілювання запитів? [💡11/100]
...
### Що таке Deadlock? Як їх уникати? [💡12/100]
...
### Партиціювання та шардування [🔥38/100]
...
**Схема "зірка" vs "сніжинка" (dimensional modeling).** Канонічний спосіб організації OLAP-сховища -
центральна таблиця фактів (вимірювані події: продажі, кліки) з зовнішніми ключами на таблиці
вимірів (dimensions: час, товар, регіон, клієнт).

- **Зірка (star)** - виміри **денормалізовані**: один плаский рядок на вимір. Менше JOIN-ів,
  швидші аналітичні запити - ціною надлишковості. Дефолт для OLAP, де читання важливіше за запис.
- **Сніжинка (snowflake)** - виміри **нормалізовані** на підтаблиці (товар → категорія → відділ).
  Менше дублювання, але більше JOIN-ів на запит. На відміну від OLTP, де нормалізація - норма,
  в OLAP за неї платять швидкістю.
