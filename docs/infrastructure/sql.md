...
**Теорія ключів.** Будь-який набір стовпців, значення яких унікальні для всіх записів
(група значень за всіма стовпцями, а не в кожному окремо), - це **суперключ**. Якщо при
вилученні будь-якого стовпця унікальність втрачається, це **потенційний (candidate)
ключ** - мінімальний унікальний набір. Потенційних ключів буває кілька; один із них
обирають основним - це **первинний ключ**. Він завжди один, може бути простим або
складеним (кілька стовпців) і не обов'язково зветься `id`.

За походженням розрізняють:

- **Природний (natural) ключ** - потенційний ключ, що вже природно є в даних (email,
  ISBN, номер договору).
- **Сурогатний (surrogate) ключ** - окремий стовпець зі згенерованими унікальними
  значеннями, коли природного ключа немає або він незручний (наприклад, складений із
  п'яти стовпців). Генерують псевдовипадково (`uuid4`, доступний ще до звернення в БД)
  або автоінкрементом (потребує звернення в БД).

**Немонотонність автоінкремента.** Згенеровані номери не зобов'язані йти поспіль і
навіть за зростанням: при видаленні записів номери не перевикористовуються, при
конкурентних транзакціях номери видаються одночасно (а частина транзакцій може
відкотитися), а деякі СУБД видають номери блоками в межах сесії. Відстежувати це було б
неефективно, тож база не намагається гарантувати порядок. Порівняння `uuid4`/`uuid7`
проти автоінкременту - у [`infrastructure/database.md`](database.md).
...
**`EXISTS` для перевірки існування.** Корельований підзапит `EXISTS (SELECT 1 ...)` повертає
`TRUE`, щойно знаходить перший відповідний рядок, і одразу зупиняється. Для відповіді "чи є
хоча б один рядок" це ефективніше за `COUNT(*) > 0`, який змушує БД порахувати всі відповідні
рядки. `NOT EXISTS` симетрично перевіряє відсутність; обидва добре працюють з індексом за
умовою підзапиту.

```sql
-- existence check: stops at the first matching row, does not count them all
