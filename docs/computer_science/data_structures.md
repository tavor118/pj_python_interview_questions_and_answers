...
*Summary*
> **Deque (двостороння черга)** - це структура даних, яка дозволяє ефективно додавати й видаляти елементи з обох кінців (на відміну від звичайної черги).

`deque` (скорочення від "double-ended queue") реалізовано в модулі `collections` і забезпечує **O(1)** операції додавання та видалення елементів зліва і справа, що вигідно порівняно з `list`, у якому вставка або видалення з початку списку має складність **O(n)**. Вона має схожий інтерфейс до звичайного списку, але з додатковими методами, такими як `appendleft()` і `popleft()` для роботи з лівим кінцем черги.

Deque зручно використовувати у задачах, де потрібно підтримувати чергу або стек з обмеженням розміру, або реалізовувати ковзаюче вікно. Також Deque безпечний для використання в багатопотокових середовищах для простих операцій (без блокувань), що робить його зручним у патернах продюсер-споживач.
