...
> серверу достатньо звірити підпис у памʼяті.
...
*Summary*
> **WebSocket** - мережевий протокол поверх TCP, що тримає постійне двостороннє зʼєднання 
> між клієнтом і сервером.
...
- Зʼєднання починається зі звичайного HTTP-запиту з заголовком `Upgrade: websocket`.
- Сервер відповідає `101 Switching Protocols` - і далі поверх того ж TCP-сокета тече двосторонній бінарний потік фреймів.
- Зʼєднання тримається відкритим, поки одна зі сторін не закриє його.
