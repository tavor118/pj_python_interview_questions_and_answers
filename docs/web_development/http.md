...
| Властивість | TCP | UDP |
| --- | --- | --- |
| Зʼєднання | 3-way handshake | Без зʼєднання |
...
всі stream'и HTTP/2 на цьому зʼєднанні. QUIC реалізує надійність на рівні
...
*Summary*
> HTTP/1.0 - окреме TCP-зʼєднання на запит. HTTP/1.1 - keep-alive, текстовий
> протокол, host-based virtual hosting. HTTP/2 - **бінарний** протокол з
> **мультиплексуванням** запитів у межах одного TCP-зʼєднання та стисненням
...
Окреме TCP-зʼєднання на кожен запит. `Connection: keep-alive` - опційне
...
- Обовʼязковий заголовок `Host` → virtual hosting.
...
повільний, наступні чекають. Браузери обходять це паралельними TCP-зʼєднаннями
(~6 на хост).
...
- **Мультиплексування streams** на одному TCP-зʼєднанні: десятки одночасних
...
- Connection migration: зʼєднання переживає зміну IP клієнта (Wi-Fi → 4G).
...
### Чим відрізняються HTTP і HTTPS [❄️2/100]
...
### Аутентифікація та авторизація [❄️4/100]
...
### Що швидше перевірити при авторизації - JWT чи сесію, і чому? [❄️2/100]
...
### Що таке WebSocket і де може використовуватися? [❄️3/100]
...
### Що таке JWT (JSON Web Token) [❄️6/100]
...
> даними інцидентів. Це стандартний чеклист для співбесід і ревʼю безпеки.
...
5. **A05 Injection** - SQL/NoSQL/command/LDAP-інʼєкції, а також XSS. Захист - параметризовані запити, екранування, валідація вводу.
