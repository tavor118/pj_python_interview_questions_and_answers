...
### Контекстний менеджер (Context manager) [🔥61/100]
...
### Що таке простори імен в Python? LEGB [💡15/100]

**Простір імен (namespace)** - це система імен, яка використовується для забезпечення 
унікальності найменування всіх об'єктів в програмі з метою уникнення можливих конфліктів. 
У Python ці простори імен реалізовані як словники з ім'ям у якості ключа та об'єктом 
в якості значення. 
В результаті різні простори можуть давати своїм об'єктам однакові імена.

LEGB визначає порядок, у якому інтерпретатор Python шукає значення для змінних. 
Спочатку він шукає у локальній області видимості, потім в області навколишньої функції 
(якщо це вкладена функція), потім в глобальній області видимості та нарешті 
в вбудованій області видимості. 
Цей порядок забезпечує правильність виконання коду та можливість використання змінних 
у правильних контекстах.

Простори імен в Python зберігаються подібно словнику, де ключем є простір імен, 
а значенням - адреса об'єкта. 
Існують 4 типи просторів імен в Python:
...
### Що таке локальні та глобальні змінні в Python? `global`, `nonlocal` [💡29/100]
