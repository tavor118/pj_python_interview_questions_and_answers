...
### Що станеться, якщо помилку не обробить блок `except` [❄️3/100]
...
### Що робити, якщо потрібно перехопити виняток, виконати дії і знову викликати цей самий виняток [❄️2/100]
...
### Для чого може використовуватися конструкція `try...finally` без `except` [❄️4/100]
...
### Чи спрацює блок `finally`, якщо в `try` є `return`? [❄️8/100]
...
> і лише потім повертає управління у місце виклику.
...
3. Тільки після цього у місце виклику повертається `"value"`.
...
функція поверне значення замість того, щоб виняток дійшов до місця виклику. Це майже
...
### Що таке ланцюжок винятків [❄️1/100]
...
### `ExceptionGroup` та `except*` (Python 3.11) [❄️1/100]
...
### Для чого потрібен блок `else` [💡12/100]
...
### Що робить `contextlib.suppress` [❄️4/100]
...
### Що можна передати у конструктор винятку [❄️3/100]
...
### Exception Object (Об'єкт винятку) [❄️2/100]
...
### Виключення vs помилки компіляції [❄️1/100]
...
### В яких випадках можна обробити `SyntaxError` [❄️2/100]
...
### Чи можна створювати власні виключення [❄️3/100]
