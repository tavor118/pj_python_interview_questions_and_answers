...
**Абсолютні й відносні шляхи, поточний каталог**

**Абсолютний шлях** веде від кореня файлової системи й однозначний незалежно від стану
процесу (`/usr/bin/python`); **відносний** (`.venv/bin/python`, `../file.dat`)
обчислюється відносно **поточного каталогу** процесу. Поточний каталог не пов'язаний із
розташуванням коду - він задається процесу окремо, успадковується від батьківського,
читається через `os.getcwd()` і змінюється `os.chdir()` (у bash - `pwd`/`cd`). Тому якщо
`/opt/app/myapp.py` робить `open("filename")`, запущений з каталогу `/root`, файл
шукатиметься як `/root/filename`. Змінювати поточний каталог через `os.chdir()` не
рекомендують; для надійних шляхів беруть `tempfile` (тимчасові файли),
`importlib.resources` (дані пакета), стандартні місця ОС (`~/.config`) або приймають шлях
через `sys.argv`.
...
**Запуск програм: `argv`, `PATH`, `subprocess`**

Нульовий елемент `sys.argv` - сама програма так, як її запускали. На рівні ОС запуск
різниться: **Linux** приймає аргументи масивом і шукає файл у `PATH`; **Windows** приймає
один рядок (його розбирає сама програма) і шукає файл також у поточному каталозі. У
`subprocess.Popen` запуск задають списком або рядком:

```python
import subprocess

subprocess.Popen(["ls", "-l", "/tmp"])      # list -> argv directly, no shell
subprocess.Popen("ls -l /tmp", shell=True)  # string parsed by /bin/sh (cmd.exe on Windows)
```

Список потрапляє в `argv` напряму; рядок передається ОС як є (тож поведінка різниться між
системами). `shell=True` додає до команди шлях до `cmd.exe` чи `sh` (шлях до `sh`
захардкоджено як `/bin/sh`, вибір оболонки користувача не враховується). Частина команд
вбудована в оболонку: `cd` змінює поточний каталог лише свого процесу й не може бути
окремою програмою (див. [`infrastructure/server.md`](../infrastructure/server.md)).
...
**Логування в архітектурі застосунку.** Підсистема логування проходить через усі шари,
включно зі сторонніми модулями, але не є частиною основної логіки - вона лише збирає
діагностику.

- **Стандартний механізм.** Доступ до запису логів потрібен будь-якій частині
  застосунку, тому використовують один стандартний механізм - модуль `logging`.
  Сторонній модуль логування має інтегруватися з `logging`, бо зовнішні бібліотеки
  знають лише про нього. Бібліотека сама не повинна використовувати сторонні модулі
  логування.
- **Ієрархія логерів.** Щоб вмикати чи вимикати логи групами, використовують ієрархію
  логерів через імена з крапками. Якщо немає кращої ідеї про структуру - беруть ім'я
  поточного модуля: `logger = logging.getLogger(__name__)`.
- **Налаштування - в інфраструктурному шарі.** Конфігурацію логування роблять там, де
  налаштовують і запускають застосунок (на початку `main`). Бібліотека не повинна
  налаштовувати логування - лише отримати логер і використовувати його.
- **Logger проти handler.** Логер - інтерфейс для запису, `handler` - детальне
  налаштування поведінки (куди відправляти: файл, потоки виводу, черга). У стандартних
  хендлерів є `formatter` (текстове представлення) і `filter` (тонший відбір, ніж за
  рівнями).
- **Куди писати.** GUI-застосунки (без терміналу) пишуть у файл або системний журнал;
  консольні скрипти можуть писати у `stderr`. Автономні серверні застосунки працюють
  довго, інколи в кількох копіях - запис у файл для них незручний (ротація, кілька
  процесів), тому стандартний спосіб - писати у `stdout`/`stderr` і дати зовнішній
  системі збирати логи (`journald` при `systemd`, сам `docker`, далі `fluentd`/ELK). Це
  підхід 12-factor.
