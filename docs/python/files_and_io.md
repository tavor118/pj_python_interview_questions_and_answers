...
### Що таке файловий об'єкт [❄️1/100]
...
### В чому відмінність текстових і бінарних файлів [❄️1/100]
...
### Як користуватися функцією `open` [💡13/100]
...
### З чого складається процес закриття файлів [💡16/100]
...
### Що роблять методи `tell` і `seek` [❄️1/100]
...
### Що роблять `StringIO` і `BytesIO` [❄️1/100]
...
### Чи є файлові об'єкти контекстними менеджерами [❄️9/100]

Так, вони є.



### В чому різниця між буферизованим і небуферизованим введенням-виведенням [❄️3/100]
...
### В чому різниця між блокуючим і неблокуючим введенням-виведенням [❄️4/100]
...
### json.dumps / json.dump, json.loads / json.load [❄️5/100]
...
### Що робити, якщо потрібно серіалізувати дані, які не підтримуються стандартним модулем json [❄️3/100]

Можна використовувати `pickle` або розширити класи `JSONEncoder` і `JSONDecoder`.



### pickle.dumps / pickle.dump, pickle.loads / pickle.load [❄️2/100]
...
### Перелік файлів у директорії [❄️2/100]
